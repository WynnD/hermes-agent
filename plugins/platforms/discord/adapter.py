...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                # Always ignore our own messages, except explicit voice handoff
                # packets posted by the voice-agent bridge. Those are a local
                # transport: Discord is the bus, Hermes is the consumer. Keep
                # the prefix narrow so normal Hermes replies never self-trigger.
                is_own_voice_handoff = False
                if message.author == self._client.user:
                    content = (getattr(message, "content", "") or "").lstrip()
                    if not content.startswith("[VOICE_HANDOFF]"):
                        return
                    is_own_voice_handoff = True
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else _discord_attachment_thread_title(message)
        if len(content) > 80:
    ...
    async def rename_auto_thread_from_attachment_processing(
        ...
        *,
        ...
        try:
            ...
            logger.debug("[%s] Failed to rename attachment auto-thread: %s", self.name, exc)
            return False
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        auto_threaded_from_attachment = False
        if not is_thread and not isinstance(message.channel, discord.DMChannel):
            ...
            if auto_thread and not skip_thread and not is_voice_linked_channel and not is_reply_message:
                ...
                if thread:
                    ...
                    auto_threaded_from_attachment = bool(all_attachments)
                    self._threads.mark(thread_id)
        ...
        def _append_attachment_note(note: str) -> None:
            """Surface attachment handling failures to the agent instead of failing silently."""
            nonlocal pending_text_injection
            if pending_text_injection:
                pending_text_injection = f"{pending_text_injection}\n\n{note}"
            else:
                pending_text_injection = note
        ...
        for att in all_attachments:
            ...
            if content_type.startswith("image/"):
                ...
                if not in_allowlist and not allow_any_attachment:
                    ...
                    display_name = att.filename or "attachment"
                    display_name = re.sub(r'[^\w.\- ]', '_', display_name)
                    _append_attachment_note(
                        f"[The user sent an attachment: '{display_name}', "
                        f"but Discord attachment ingestion skipped it because "
                        f"its type is not allowlisted (extension: {ext or 'unknown'}, "
                        f"content type: {content_type}). Enable discord.allow_any_attachment "
                        f"or add support for this file type to read it.]"
                    )
                else:
                    ...
                    if max_doc_bytes and att.size and att.size > max_doc_bytes:
                        ...
                        display_name = att.filename or f"document{ext or ''}"
                        display_name = re.sub(r'[^\w.\- ]', '_', display_name)
                        _append_attachment_note(
                            f"[The user sent an attachment: '{display_name}', "
                            f"but Discord attachment ingestion skipped it because "
                            f"it is {att.size} bytes and the configured cap is "
                            f"{max_doc_bytes} bytes. Increase discord.max_attachment_bytes "
                            f"or set it to 0 to cache this file.]"
                        )
                    else:
                        try:
        ...
        if thread_id:
            self._threads.mark(thread_id)
        if auto_threaded_channel and auto_threaded_from_attachment:
