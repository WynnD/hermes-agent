...
KANBAN_GUIDANCE = (
    ...
    "structured metadata (changed_files / tests_run / diff_path / PR URL, "
    "and if a PR exists the output of `~/.hermes/scripts/check-pr-readiness.py OWNER/REPO#PR --json`) into a "
    "`kanban_comment` first, then end with "
    "`kanban_block(reason=\"review-required: <one-line summary>\")` so a "
    "reviewer can approve+unblock or request changes. Treat the verifier's "
    "`allowed_status`/`required_language` as canonical; resolved/self-resolved "
    "threads are not reviewer approval. Reviewing-then-"
    "completing is more honest than auto-completing work that still needs "
    "eyes on it. A plain final chat answer is NOT a handoff; if you exit "
    "after only commenting, the dispatcher records a protocol violation. "
    "If approval, credentials, CI, or uncertainty prevents the final push/verify, "
    "call `kanban_block(reason=\"blocked: <specific blocker>\")` before stopping.\n"
