...
def link_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> None:
    ...
    with write_txn(conn):
        ...
        if not _parent_satisfies_child(
            conn,
            parent_id=parent_id,
            child_assignee=child["assignee"] if child else None,
        ):
            conn.execute(
...
def recompute_ready(conn: sqlite3.Connection) -> int:
    """Promote ``todo`` tasks when all parents satisfy the child lane.

    Most tasks require parents to be ``done`` or ``archived``. Reviewer tasks
    may also advance when parents are blocked with a ``review-required:`` handoff
    so review cards can inspect PRs without marking implementation cards done.
...
def claim_task(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        # parent does not satisfy the child lane. Most children require parents
        # to be done/archived. Reviewer children may run when an implementation
        # parent is blocked with `review-required:` because that blocked handoff
        # is the review trigger, not a failure. This is the single enforcement
        ...
        # back to 'todo' here — recompute_ready will re-promote when the parents
        # actually satisfy. See RCA at
...
def block_task(
    ...
    *,
    ...
    # Blocking with a review-required handoff may satisfy reviewer children.
    # Recompute after the blocking transaction commits so the next dispatch tick
    # can claim review tasks immediately.
    recompute_ready(conn)
    return True
...
def unblock_task(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    with write_txn(conn):
        ...
        # Re-gate before flipping 'blocked' back to 'ready'. Unconditionally
        # setting status='ready' here bypasses the dependency invariant (the
        # dispatcher trusts that column); if parents are still unsatisfied the
        # task must wait in 'todo' until recompute_ready picks it up. Reviewer
        # children use the same review-required handoff exception as dispatch.
