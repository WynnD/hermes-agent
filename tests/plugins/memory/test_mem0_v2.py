...
class TestMem0Defaults:
    ...
    def test_base_url_enables_self_hosted_without_api_key(self, monkeypatch, tmp_path):
        monkeypatch.delenv("MEM0_API_KEY", raising=False)
        monkeypatch.setenv("MEM0_BASE_URL", "https://mem0.local")
    ...
    def test_config_user_id_overrides_gateway_user_id_for_canonical_scope(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        (tmp_path / "mem0.json").write_text(json.dumps({
            "base_url": "https://mem0.local",
            "user_id": "wynn",
            "agent_id": "hermes",
        }))

        provider = Mem0MemoryProvider()
        provider.initialize("test", user_id="184442182348963840")

        assert provider._user_id == "wynn"

    def test_base_url_sync_turn_disables_inference_to_avoid_junk_captures(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        (tmp_path / "mem0.json").write_text(json.dumps({
            "base_url": "https://mem0.local",
            "user_id": "wynn",
