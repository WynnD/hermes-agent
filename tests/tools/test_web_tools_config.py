...
class TestBackendSelection:
    ...
    def test_config_native(self):
        """web.backend=native in config → 'native' with no API keys."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={"backend": "native"}):
    ...
    def test_fallback_no_keys_defaults_to_native(self):
        """No keys, no config → 'native' local extraction backend."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}):
            assert _get_backend() == "native"
...
class TestCheckWebApiKey:
    ...
    def test_native_extract_backend_available_without_api_keys(self):
        from tools.web_tools import check_web_extract_backend
        with patch("tools.web_tools._load_web_config", return_value={"extract_backend": "native"}):
            assert check_web_extract_backend() is True
